...
- `scripts/github-review.py` is manual-only deterministic offline tooling: backfill saved Thunderstore HTML, export bounded evidence batches, and apply validated decisions. It is not imported or invoked by tracker, reports, verification, `manual-pass.py`, or unattended runs; tracked review state changes only through explicit manual apply.
...
- Keep category filtering shared and source-derived: normalized card metadata and dropdown options must match grouped report categories, category chips must be native buttons, and all filters must compose in one predicate. Search every grouped title, displayed/credited creator, provider author, and retained short description; search only filters the matching set and never changes the selected sort order. Paginate that complete filtered and selected-sort-ordered set client-side at exactly 100 cards per page in the same HTML file, count pinned cards toward the limit, reset to page 1 after filter/sort/search changes, and bind the controls, search indexes, search program, and pagination script in strict verification. Keep the toolbar outside the title header so desktop viewport-sticky positioning is not parent-bounded; disable sticky positioning at 520px and below so mobile uses normal document flow.
...
- Publish the configured `/mod-tracking/` tree as one preflighted root batch: replace with rollback protection, commit once, deploy once, and verify the landing page plus all five game URLs. Publish the same staged tree to the GitHub Pages branch with `scripts/publish-github.py`, which is the canonical host for these reports; it force-pushes a single orphan commit and skips the push entirely when the branch already carries the identical tree.
