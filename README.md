...
Strict CLI verification checks configured and actual raw-page scope, Thunderstore listing-manifest/snapshot agreement, valid terminal short pages, stale-page absence, source-record uniqueness and required fields, both report artifacts, exact canonical card counts, required controls and sort/search metadata, source-derived category options/card metadata/buttons, canonical category/pagination/search JavaScript, selected-sort preservation, sticky responsive toolbar behavior, independently recomputed author reputation, exact rendered author identity/canonical/display-name/total/tier/mod-count/first-publication/accessibility metadata, parsed latest-data timestamp metadata and visible text, matching hotlinked/local bytes, and absence of embedded `data:image/` content from the publication artifact.
